...
# List of card values
# J is now Joker with lower value card
card_list = ['J'] + [str(x) for x in range(2,10)] + ['T', 'Q', 'K', 'A']

def use_joker(hand_dict):
    # Nothing to do if no Jokers or all 5 cards are Jokers
    if hand_dict['J'] == 0 or hand_dict['J'] == 5:
        return hand_dict
    else:
        num_jokers = hand_dict['J']
        hand_dict['J'] = 0
        # Find highest card with highest number of cards in hand
        best_card = max(reversed(hand_dict), key=hand_dict.get)
        # Use Joker to add to the best card
        hand_dict[best_card] += num_jokers
        return(hand_dict)
...
for game in games:
    ...
    for card in game.hand:
        hand_dict[card] += 1
    # Use Joker cards to improve hand type
    hand_dict = use_joker(hand_dict)
