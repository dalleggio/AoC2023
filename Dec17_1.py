...
def get_neighbors(r, c, dir, rl, visited):
    ...
    if dir == 'U':
        # up neighbor
        if rl < 3 and r > 0 and (r - 1, c, 'U') not in visited:
            neighbors.append((r - 1, c, 'U', rl + 1))
        # left neighbor
        if c > 0 and (r, c - 1, 'L') not in visited:
            neighbors.append((r, c - 1, 'L', 1))
        # right neighbor
        if c < num_cols - 1 and (r, c + 1, 'R') not in visited:
            neighbors.append((r, c + 1, 'R', 1))
            
    elif dir == 'D':
        # down neighbor
        if rl < 3 and r < num_rows - 1 and (r + 1, c, 'D') not in visited:
        ...
        # right neighbor
        if rl < 3 and c < num_cols - 1 and (r, c + 1, 'R') not in visited:
            neighbors.append((r, c + 1, 'R', rl + 1))
        # up neighbor
        if r > 0 and (r - 1, c, 'U') not in visited:
            neighbors.append((r - 1, c, 'U', 1))
        # down neighbor
        if r < num_rows - 1 and (r + 1, c, 'D') not in visited:
            neighbors.append((r + 1, c, 'D', 1))

    elif dir == 'L':
        # left neighbor
        if rl < 3 and c > 0 and (r, c - 1, 'L') not in visited:
